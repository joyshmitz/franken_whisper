...
| Robot mode (NDJSON output) | None (novel) | `robot` | Done | Structured NDJSON line-oriented output with schema version 1.0.0. Coverage includes lifecycle events (`run_start`, `stage`, `run_complete`, `run_error`), discovery/diagnostics (`backends.discovery`, `health.report`), routing-history output (`routing_decision` via `robot routing-history`), and speculative streaming events (`transcript.partial`, `transcript.confirm`, `transcript.retract`, `transcript.correct`, `transcript.speculation_stats`). Required fields are enforced per event type. |
